...
type AppPlan = { name: string; tagline: string; purpose: string; targetUser: string; features: string[]; screens: string[] };
type BuildSpec = {
...
function fallback(plan: AppPlan): BuildSpec {
  const entities = [
    ...
    { name: "Item", fields: ["id", "title", "status", "createdAt", "updatedAt"] },
  ];
  return {
    productSummary: `${plan.name} is for ${plan.targetUser}. ${plan.purpose}`,
    userStories: plan.features.slice(0, 5).map((f) => `As a user, I want to ${f.charAt(0).toLowerCase()}${f.slice(1)} so I can complete the app's main job.`),
    dataEntities: entities,
    screenDetails: plan.screens.slice(0, 6).map((name) => ({ name, purpose: `Support the ${name.toLowerCase()} part of the workflow.`, actions: ["View relevant information", "Create or update data when needed"] })),
    acceptanceCriteria: [
...
function parse(text: string): BuildSpec | null {
  try {
    ...
    if (typeof p.productSummary !== "string" || !Array.isArray(p.userStories) || !Array.isArray(p.dataEntities) || !Array.isArray(p.screenDetails) || !Array.isArray(p.acceptanceCriteria) || typeof p.agentPrompt !== "string") return null;
    return {
      productSummary: p.productSummary,
      userStories: p.userStories.filter((x): x is string => typeof x === "string").slice(0, 6),
      dataEntities: p.dataEntities.filter((x): x is {name:string;fields:string[]} => !!x && typeof x.name === "string" && Array.isArray(x.fields)).slice(0, 6).map(x => ({ name: x.name, fields: x.fields.filter((f): f is string => typeof f === "string").slice(0, 8) })),
      screenDetails: p.screenDetails.filter((x): x is {name:string;purpose:string;actions:string[]} => !!x && typeof x.name === "string" && typeof x.purpose === "string" && Array.isArray(x.actions)).slice(0, 6).map(x => ({ name: x.name, purpose: x.purpose, actions: x.actions.filter((a): a is string => typeof a === "string").slice(0, 4) })),
      acceptanceCriteria: p.acceptanceCriteria.filter((x): x is string => typeof x === "string").slice(0, 7),
...
export async function POST(request: Request) {
  const body = await request.json() as { idea?: unknown; history?: unknown; plan?: unknown };
